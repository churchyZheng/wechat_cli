...
var textCallbackGuardAddr = baseAddr.add(0x24669E8);
var textCallbackSkipAddr = baseAddr.add(0x24669F4);
var textCleanupGuardAddr = baseAddr.add(0x2466B30);
var textCleanupSkipAddr = baseAddr.add(0x2466B98);

const TEXT_REQ_TEMPLATE_SIZE = 0x200;
const TEXT_MSG_TEMPLATE_SIZE = 0x200;
const TEXT_PAYLOAD_TEMPLATE_SIZE = 0x200;
const TEXT_CALLBACK_SLOT_OFFSET = 0x98;
const TEXT_PAYLOAD_CGI_PTR_OFFSET = 0x18;
const TEXT_PAYLOAD_SELF_PTR1_OFFSET = 0xB8;
const TEXT_PAYLOAD_SELF_TARGET1_OFFSET = 0xC0;
const TEXT_PAYLOAD_SELF_PTR2_OFFSET = 0x190;
const TEXT_PAYLOAD_SELF_TARGET2_OFFSET = 0x198;

var patchTextProtobufByte = null;
var patchTextProtobufDeleteByte = null;
var textCgiAddr = ptr(0);
var sendTextMessageAddr = ptr(0);
var textMessageAddr = ptr(0);
var textReqTemplateAddr = ptr(0);
var textMsgTemplateAddr = ptr(0);
var textPayloadTemplateAddr = ptr(0);
var textProtoX1PayloadAddr = ptr(0);
var textCallbackObjectAddr = ptr(0);
var lastOriginalTextReqAddr = ptr(0);
var lastOriginalTextMsgAddr = ptr(0);
var lastOriginalTextCallbackAddr = ptr(0);
var textTemplateReady = false;
var textProtoPending = false;
var req2bufHookInstalled = false;
var capturedTextTaskId = 0;


// 双方公共使用的地址
var triggerX0 = ptr(0);
var triggerX1Payload = ptr(0);
...
// -------------------------发送文本消息分区-------------------------
function clearTextSendState() {
    taskIdGlobal = 0;
    receiverGlobal = "wxid_";
    contentGlobal = "";
    senderGlobal = "wxid_";
    atUserGlobal = "";
    textProtoPending = false;
    sendMsgType = "";
}

function safeAttach(address, label, callbacks) {
    if (!address || address.isNull()) {
        console.warn("[!] " + label + " 地址为空，跳过.");
        return false;
    }
    try {
        ...
        console.warn("[!] 挂载 " + label + " 失败: " + e);
        return false;
    }
}

function writeCodeBytes(address, byteArray) {
    let bytes = byteArray;
...
function safeNopPatch(address, label) {
    if (!address || address.isNull()) {
        console.warn("[!] " + label + " 地址为空，跳过.");
        return false;
    }
    if (address.equals(baseAddr)) {
        console.warn("[!] " + label + " 偏移仍是 0x0，占位地址: " + address + "，跳过.");
        return false;
    }

    const range = Process.findRangeByAddress(address);
    if (range === null || range.protection.indexOf("x") === -1) {
        console.warn("[!] " + label + " 地址不是可执行指令，跳过: " + address);
        return false;
    }

    try {
        writeCodeBytes(address, [0x1f, 0x20, 0x03, 0xd5]);
        console.log("[+] Patching " + label + " " + address + " 成功.");
        return true;
    } catch (e) {
        console.warn("[!] Patching " + label + " 失败: " + e);
        return false;
    }
}

function restoreOriginalInstruction(address, byteArray) {
    if (byteArray === null) {
        return;
    }
    writeCodeBytes(address, byteArray);
}

function copyTemplateMemory(dst, src, size, label) {
    const range = Process.findRangeByAddress(src);
    let readableSize = size;
    if (range !== null) {
        const maxReadable = range.base.add(range.size).sub(src).toInt32();
        readableSize = Math.min(size, maxReadable);
    }

    if (readableSize <= 0) {
        console.warn("[!] " + label + " 模板可读长度无效: " + src);
        return false;
    }

    try {
        const bytes = src.readByteArray(readableSize);
        if (bytes === null) {
            console.warn("[!] " + label + " 模板读取为空: " + src);
            return false;
        }
        dst.writeByteArray(new Uint8Array(size));
        dst.writeByteArray(bytes);
        return true;
    } catch (e) {
        console.warn("[!] " + label + " 模板复制失败: " + src + " -> " + e);
        return false;
    }
}

function relocateTemplatePointers(dst, src, size, label) {
    if (!dst || dst.isNull() || !src || src.isNull()) {
        return;
    }

    const srcEnd = src.add(size);
    const slotSize = Process.pointerSize;
    for (let off = 0; off + slotSize <= size; off += slotSize) {
        try {
            const value = src.add(off).readPointer();
            if (value.isNull()) {
                continue;
            }
            if (value.compare(src) >= 0 && value.compare(srcEnd) < 0) {
                dst.add(off).writePointer(dst.add(value.sub(src).toInt32()));
            }
        } catch (e) {
            console.warn("[!] " + label + " 指针重定位失败 off=0x" + off.toString(16) + ": " + e);
        }
    }
}

function cacheTextTemplateObjects(originalReqAddr) {
    if (originalReqAddr.isNull()) {
        console.warn("[!] 原始文本请求对象为空，无法缓存模板.");
        return false;
    }

    const originalMsgAddr = originalReqAddr.add(0x28).readPointer();
    if (originalMsgAddr.isNull()) {
        console.warn("[!] 原始文本消息对象为空，无法缓存模板.");
        return false;
    }

    let callbackObjAddr = ptr(0);
    try {
        callbackObjAddr = originalMsgAddr.add(TEXT_CALLBACK_SLOT_OFFSET).readPointer();
    } catch (e) {
        console.warn("[!] 读取文本回调对象失败: " + e);
        return false;
    }

    if (callbackObjAddr.isNull()) {
        console.warn("[!] 原始文本回调对象为空，无法缓存模板.");
        return false;
    }

    if (!copyTemplateMemory(textReqTemplateAddr, originalReqAddr, TEXT_REQ_TEMPLATE_SIZE, "textReqTemplateAddr")) {
        return false;
    }
    if (!copyTemplateMemory(textMsgTemplateAddr, originalMsgAddr, TEXT_MSG_TEMPLATE_SIZE, "textMsgTemplateAddr")) {
        return false;
    }

    lastOriginalTextReqAddr = originalReqAddr;
    lastOriginalTextMsgAddr = originalMsgAddr;
    lastOriginalTextCallbackAddr = callbackObjAddr;
    textTemplateReady = true;
    console.log("[+] 已缓存真实文本模板对象: req=" + originalReqAddr + " msg=" + originalMsgAddr + " callback=" + callbackObjAddr);
    return true;
}

function prepareTextRequestObjects() {
    if (!textTemplateReady) {
        console.warn("[!] 真实文本模板尚未准备好.");
        return false;
    }

    if (!copyTemplateMemory(sendTextMessageAddr, textReqTemplateAddr, TEXT_REQ_TEMPLATE_SIZE, "sendTextMessageAddr")) {
        return false;
    }
    if (!copyTemplateMemory(textMessageAddr, textMsgTemplateAddr, TEXT_MSG_TEMPLATE_SIZE, "textMessageAddr")) {
        return false;
    }

    relocateTemplatePointers(sendTextMessageAddr, textReqTemplateAddr, TEXT_REQ_TEMPLATE_SIZE, "sendTextMessageAddr");
    relocateTemplatePointers(textMessageAddr, textMsgTemplateAddr, TEXT_MSG_TEMPLATE_SIZE, "textMessageAddr");

    // Req2Buf 的 x24+0x60 期待的是一个干净的请求节点，不能带上旧树链接。
    ...
    textMessageAddr.add(TEXT_CALLBACK_SLOT_OFFSET).writePointer(lastOriginalTextCallbackAddr);
    textCallbackObjectAddr = lastOriginalTextCallbackAddr;

    console.log("[+] 已从真实模板构建干净文本请求节点. req=" + sendTextMessageAddr + " msg=" + textMessageAddr + " callbackObj=" + textCallbackObjectAddr);
    return true;
}

function normalizeTextPayloadTemplate(payloadAddr) {
    if (!payloadAddr || payloadAddr.isNull()) {
        return false;
    }

    try {
        payloadAddr.add(TEXT_PAYLOAD_CGI_PTR_OFFSET).writePointer(textCgiAddr);
        payloadAddr.add(TEXT_PAYLOAD_SELF_PTR1_OFFSET).writePointer(payloadAddr.add(TEXT_PAYLOAD_SELF_TARGET1_OFFSET));
        payloadAddr.add(TEXT_PAYLOAD_SELF_PTR2_OFFSET).writePointer(payloadAddr.add(TEXT_PAYLOAD_SELF_TARGET2_OFFSET));
        console.log("[+] 已修正文本 Payload 模板关键指针: cgi=" +
            payloadAddr.add(TEXT_PAYLOAD_CGI_PTR_OFFSET).readPointer() +
            " self1=" + payloadAddr.add(TEXT_PAYLOAD_SELF_PTR1_OFFSET).readPointer() +
            " self2=" + payloadAddr.add(TEXT_PAYLOAD_SELF_PTR2_OFFSET).readPointer());
        return true;
    } catch (e) {
        console.warn("[!] 修正文本 Payload 模板失败: " + e);
        return false;
    }
}
...
function setupSendTextMessageDynamic() {
    ...
    sendTextMessageAddr = Memory.alloc(TEXT_REQ_TEMPLATE_SIZE);
    textMessageAddr = Memory.alloc(TEXT_MSG_TEMPLATE_SIZE);
    textReqTemplateAddr = Memory.alloc(TEXT_REQ_TEMPLATE_SIZE);
    textMsgTemplateAddr = Memory.alloc(TEXT_MSG_TEMPLATE_SIZE);
    textPayloadTemplateAddr = Memory.alloc(TEXT_PAYLOAD_TEMPLATE_SIZE);
    ...
    console.log("[+] Dynamic Memory Setup Complete. - Text req/msg/template buffers ready.");
}
...
function patchTextProtoBuf() {
    safeAttach(textCallbackFuncAddr, "text callback", {
        onEnter: function (args) {
            var firstValue = this.context.sp.readU32();
            if (textProtoPending && taskIdGlobal !== 0 && firstValue === taskIdGlobal) {
                if (patchTextProtobufAddr.readU32() !== 3573751839) {
                    writeCodeBytes(patchTextProtobufAddr, [0x1f, 0x20, 0x03, 0xd5]);
                    writeCodeBytes(patchTextProtobufDeleteAddr, [0x1f, 0x20, 0x03, 0xd5]);
                }
            } else {
                if (patchTextProtobufAddr.readU32() === 3573751839) {
...
function triggerSendTextMessage(taskId, receiver, content, atUser) {
    ...
    if (!triggerX0 || triggerX0.isNull() || !triggerX1Payload || triggerX1Payload.isNull()) {
        console.error("[!] 尚未捕获到真实 MMStartTask 的 X0/X1，请先手动发送一条普通文本消息.");
        clearTextSendState();
        return "fail";
    }
    if (!textTemplateReady) {
        console.error("[!] 尚未捕获到真实文本模板，请先手动发送一条普通文本消息.");
        clearTextSendState();
        return "fail";
    }

    textProtoPending = true;
    textPayloadTemplateAddr.writeU32(taskIdGlobal);
    if (!normalizeTextPayloadTemplate(textPayloadTemplateAddr)) {
        clearTextSendState();
        return "fail";
    }

    triggerX1Payload = textPayloadTemplateAddr;
    sendMsgType = "text";
...
function AttachSendTextProto() {
    safeAttach(sendFuncAddr, "sendFunc capture", {
        onEnter: function (args) {
            let taskId = 0;
            let cgi = "";
            try {
                taskId = this.context.x1.readU32();
                const cgiPtr = this.context.x1.add(0x18).readPointer();
                cgi = cgiPtr.readUtf8String();
            } catch (e) {
                return;
            }

            if (cgi !== "/cgi-bin/micromsg-bin/newsendmsg") {
                return;
            }
            if (!copyTemplateMemory(textPayloadTemplateAddr, this.context.x1, TEXT_PAYLOAD_TEMPLATE_SIZE, "textPayloadTemplateAddr")) {
                return;
            }
            if (!normalizeTextPayloadTemplate(textPayloadTemplateAddr)) {
                return;
            ...
            triggerX1Payload = textPayloadTemplateAddr;
            capturedTextTaskId = taskId;
            console.log("[+] 捕获到真实文本 MMStartTask，taskId=" + capturedTextTaskId + "，X0地址：" + triggerX0 + ", Payload 模板地址: " + triggerX1Payload);
        }
    });
}
...
function attachSendTextProto() {
    ...
    safeAttach(protobufAddr, "text protobuf", {
        onEnter: function (args) {
            if (!textProtoPending || taskIdGlobal === 0 || !receiverGlobal || !contentGlobal) {
                return;
            }
            ...
            clearTextSendState();
            console.log("[+] 文本 protobuf 注入完成，已清理发送状态.");
        },
    });
}
...
function attachReq2buf() {
    if (req2bufHookInstalled) {
        return;
    }

    console.log("[+] Target Req2buf enter Address: " + req2bufEnterAddr);

    safeAttach(req2bufEnterAddr, "req2buf enter", {
        onEnter: function (args) {
            const currentTaskId = this.context.x1.toUInt32();

            if (!textTemplateReady && capturedTextTaskId !== 0 && currentTaskId === capturedTextTaskId) {
                try {
                    const templateInsertAddr = this.context.x24.add(0x60);
                    const templateReqAddr = templateInsertAddr.readPointer();
                    cacheTextTemplateObjects(templateReqAddr);
                } catch (e) {
                    console.warn("[!] 捕获真实文本模板失败: " + e);
                }
            }
    ...
    safeAttach(req2bufExitAddr, "req2buf leave", {
        onEnter: function (args) {
            if (!insertMsgAddr || insertMsgAddr.isNull()) {
                return;
            }
            ...
            if (sendMsgType === "text" && textProtoPending) {
                console.log("[+] 文本发送保留状态，等待 protobuf 注入完成后再清理.");
                send({
                    type: "finish",
                });
                return;
...
function patchImgProtoBuf() {
    safeNopPatch(patchImgProtobufFunc1, "patchImgProtobufFunc1");
    safeNopPatch(patchImgProtobufFunc2, "patchImgProtobufFunc2");
    safeNopPatch(imgProtobufDeleteAddr, "imgProtobufDeleteAddr");
}
